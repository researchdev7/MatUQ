...
void fused_dpa_3dim_bwd(
    ...
    if (tgbij_ehk != NULL && tbij_ehk != NULL ) {
        for (long long int e = e_start; e < e_end; e++)
        {
            ...
            const float *go = gout_ihk + (i*H+h)*3;
            const float *o = out_ihk + (i*H+h)*3;
            float *gb = tgbij_ehk + (e*H+h)*3;
            const float *b = tbij_ehk + (e*H+h)*3;
            float g_softmax = 0;
            #pragma unroll
            for (int k = 0; k < 3; k++){
                float t = go[k]*pij;
