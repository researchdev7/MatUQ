...
void compute_lattice_third_frame(
    const float* a_ik,
    ...
    float cross_x = dy_first[i*H + k] * dz_second[i*H + k] - dz_first[i*H + k] * dy_second[i*H + k];
    float cross_y = dz_first[i*H + k] * dx_second[i*H + k] - dx_first[i*H + k] * dz_second[i*H + k];
    float cross_z = dx_first[i*H + k] * dy_second[i*H + k] - dy_first[i*H + k] * dx_second[i*H + k];

    float cross_norm = sqrtf(cross_x*cross_x + cross_y*cross_y + cross_z*cross_z);
    cross_x = cross_x / cross_norm;
    cross_y = cross_y / cross_norm;
    cross_z = cross_z / cross_norm;
    ...
    for (float n3 = -R3; n3 <= R3; n3++)
    {
        ...
        float cos_term_cross = dx_norm * cross_x + dy_norm * cross_y + dz_norm * cross_z;
        float cos_term_frame1 = dx_norm * dx_first[i*H+k] + dy_norm * dy_first[i*H+k] + dz_norm * dz_first[i*H+k];

        if (fabsf(cos_term_cross) > 1e-5){
            if (cos_term_frame1 < 0){
            dx_third[i*H+k] = -dx_norm;
            dy_third[i*H+k] = -dy_norm;
            dz_third[i*H+k] = -dz_norm;
            cos_term_cross = -cos_term_cross;
            } else {
            dx_third[i*H+k] = dx_norm;
            dy_third[i*H+k] = dy_norm;
            dz_third[i*H+k] = dz_norm;
    ...
    float cos_term_cross = dx_third[i*H+k] * cross_x + dy_third[i*H+k] * cross_y + dz_third[i*H+k] * cross_z;
    if (cos_term_cross < 0){
        dx_first[i*H+k] = -dx_first[i*H+k];
        dy_first[i*H+k] = -dy_first[i*H+k];
        dz_first[i*H+k] = -dz_first[i*H+k];
        dx_second[i*H+k] = -dx_second[i*H+k];
        dy_second[i*H+k] = -dy_second[i*H+k];
        dz_second[i*H+k] = -dz_second[i*H+k];
        dx_third[i*H+k] = -dx_third[i*H+k];
        dy_third[i*H+k] = -dy_third[i*H+k];
        dz_third[i*H+k] = -dz_third[i*H+k];
    }
}