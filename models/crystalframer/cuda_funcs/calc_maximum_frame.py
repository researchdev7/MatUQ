...
class ComputeFrame1MaximumMethodCUDA(torch.autograd.Function):
    @staticmethod
    def forward(ctx, q, k, v, a_ik, rpos_ij_e, dist2_min_e, tvecs_n, batch_i, edge_ij_e, K, dist_max, wscale, rvlen_n=None, cutoff_radius=None):
        ...
        q_tmp = q[edge_ij_e[0, :], :, :].reshape(E, H, 1, -1)
        k_tmp = k[edge_ij_e[1, :], :, :].reshape(E, H, -1, 1)
        q_dot_k = q_tmp @ k_tmp
        attn = q_dot_k[:, :, 0, 0] + maximum_value
        out, argmax = torch_scatter.scatter_max(
            attn, edge_ij_e[0, :].repeat(H, 1).T,
...
class ComputeFrame2MaximumMethodCUDA(torch.autograd.Function):
    @staticmethod
    def forward(ctx, q, k, v, a_ik, rpos_ij_e, dist2_min_e, tvecs_n, batch_i, edge_ij_e, K, dist_max, wscale, frame1, rvlen_n=None, cutoff_radius=None):
        ...
        with cp.cuda.Device(dev.index), ppe.cuda.stream(torch.cuda.current_stream(dev)):
            kernel = KernelManager.compute_second_frame_each_edge
            kernel(((E * H + bsz - 1) // bsz,), (bsz,), (
                    _to_cupy(a_ik),
                    _to_cupy(rpos_ij_e),
                    _to_cupy(dist2_min_e),
        ...
        attn = q_dot_k[:, :, 0, 0] + second_value

        out, argmax = torch_scatter.scatter_max(attn, edge_ij_e[0, :].repeat(H, 1).T, out=torch.ones((N, H), **kw)*(-1e10), dim = 0)
...
class ComputeFrame3MaximumMethodCUDA(torch.autograd.Function):
    @staticmethod
    def forward(ctx, q, k, v, a_ik, rpos_ij_e, dist2_min_e, tvecs_n, batch_i, edge_ij_e, K, dist_max, wscale, frame1, frame2, rvlen_n=None, cutoff_radius=None):
        a_ik = a_ik.to(torch.float32)
        q = q.to(torch.float32)
        k = k.to(torch.float32)
        ...
        dx_second = frame2[:, :, 0].contiguous()
        dy_second = frame2[:, :, 1].contiguous()
        dz_second = frame2[:, :, 2].contiguous()

        dev = a_ik.device

        bsz = H
        dx_third = torch.empty((E, H), **kw)
        dy_third = torch.empty((E, H), **kw)
        dz_third = torch.empty((E, H), **kw)
        third_value = torch.empty((E, H), **kw)

        seed = np.random.randint(np.iinfo(np.int64).min, np.iinfo(np.int64).max, dtype=np.int64).item()
        state_buff = torch.empty(E * H * 48, dtype=torch.uint8, device=dev)

        with cp.cuda.Device(dev.index), ppe.cuda.stream(torch.cuda.current_stream(dev)):
            kernel = KernelManager.compute_third_frame_each_edge
            kernel(((E * H + bsz - 1) // bsz,), (bsz,), (
                    _to_cupy(a_ik),
                    _to_cupy(rpos_ij_e),
                    _to_cupy(dist2_min_e),
        ...
        attn = q_dot_k[:, :, 0, 0] + third_value

        out, argmax = torch_scatter.scatter_max(attn, edge_ij_e[0, :].repeat(H, 1).T, out=torch.ones((N, H), **kw)*(-1e10), dim = 0)

        index = torch.arange(H, device = a_ik.device).repeat(N, 1)

        dx_third_max = dx_third[argmax, index]
        dy_third_max = dy_third[argmax, index]
        dz_third_max = dz_third[argmax, index]

        frame_third = torch.empty((N, H, 3), **kw)
        frame_third[:, :, 0] = dx_third_max
        frame_third[:, :, 1] = dy_third_max
        frame_third[:, :, 2] = dz_third_max
